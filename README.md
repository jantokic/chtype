...
```
npm install @chtype/codegen @chtype/query @chtype/client
```
...
chtype fixes this with three packages:
...
```typescript
// @generated by @chtype/codegen — do not edit manually
...
// users is typed as { user_id: string; name: string; score: number | null }[]
```
...
await ch.insert('users', [
  { user_id: '123', name: 'Alice' },           // updated_at is optional (DEFAULT)
  { user_id: '456', name: 'Bob', score: 42 },  // score is optional too
]);
```
...
export default defineConfig({
  ...
  output: './src/generated/schema.ts',
  bigints: false,  // map UInt64/Int64 to bigint instead of string
...
| ClickHouse-only focus | Yes | Yes | No (generic) | Yes |

## Packages

- **[@chtype/codegen](./packages/codegen)** — Schema introspection and TypeScript code generation
- **[@chtype/query](./packages/query)** — Type-safe query builder (zero dependencies)
- **[@chtype/client](./packages/client)** — Enhanced client wrapper
