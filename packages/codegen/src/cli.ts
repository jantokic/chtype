 ...
 *
 * Usage:
 *   npx @chtype/codegen generate --host http://localhost:8123 --database my_db
 *   npx @chtype/codegen generate --config chtype.config.ts
 */
...
const generateCommand = defineCommand({
  ...
  args: {
    ...
    output: {
      ...
      default: './chtype.generated.ts',
    },
    bigints: {
      type: 'boolean',
      description: 'Map UInt64/Int64 to bigint instead of string',
      default: false,
    },
    include: {
      type: 'string',
      description: 'Comma-separated table name patterns to include',
    },
    exclude: {
      type: 'string',
      description: 'Comma-separated table name patterns to exclude',
    },
    config: {
      type: 'string',
      alias: 'c',
      description: 'Path to config file (chtype.config.ts)',
    },
  },
  async run({ args }) {
    ...
    if (args.config) {
      ...
      if (!fileConfig) {
        console.error(`Failed to load config file: ${args.config}`);
        process.exit(1);
      }
      config = fileConfig;
    } else {
    ...
    try {
      ...
      for (const table of tables) {
        console.log(
          `  ${table.name} (${table.engine}, ${table.columns.length} columns)`,
        );
      }

      const commandStr = `npx @chtype/codegen generate --host ${config.connection.host} --database ${config.connection.database}`;
